...
//  | bytes | keyword             |                                     |
//  |-------|---------------------|-------------------------------------|
//  | 4     | _token              | 0x00000000 - 0xffffffff             |  --+
//  | 1     | apng                | 0 or 1                              |    |
//  | 2     | width               | 0 - 0xffff                          |    |
//  | 2     | height              | 0 - 0xffff                          |    |
//  | 1     | loopCount           | 0 - 0xff                            |    +---  16bytes
...
function BinaryPacker0x0003_pack(source) { // @arg Object - { _token, apng, width, height, loopCount, usePosterFrame, backgroundColor, frameData }
                                           // @ret Uint8Array - buffer
//{@dev
    $valid($type(source,                "Object"),        BinaryPacker0x0003_pack, "source");
    $valid($type(source._token,         "UINT32"),        BinaryPacker0x0003_pack, "source._token");
    $valid($type(source.apng,           "UINT8|Boolean"), BinaryPacker0x0003_pack, "source.apng");
    $valid($type(source.width,          "UINT16"),        BinaryPacker0x0003_pack, "source.width");
    $valid($type(source.height,         "UINT16"),        BinaryPacker0x0003_pack, "source.height");
    $valid($type(source.loopCount,      "UINT8"),         BinaryPacker0x0003_pack, "source.loopCount");
    $valid($type(source.usePosterFrame, "UINT8|Boolean"), BinaryPacker0x0003_pack, "source.usePosterFrame");
    $valid($type(source.backgroundColor,"RRGGBB"),        BinaryPacker0x0003_pack, "source.backgroundColor");
    $valid($type(source.frameData,      "APNGFrameDataArray"), BinaryPacker0x0003_pack, "source.frameData");
//}@dev

    var headLength = 8;
    var bodyLength = BinaryPacker0x0003_getBodyLength(source);
    var buffer = new Uint8Array(headLength + bodyLength);
    var cursor = global["BinaryPacker"]["writeHead"](buffer, 0, bodyLength, 0x0003);
    var frameData = source["frameData"];
    var frameDataLength = frameData.length;
    var backgroundColor = source["backgroundColor"];
    var body = [
            (source["_token"] >> 24) & 0xff,
            (source["_token"] >> 16) & 0xff,
            (source["_token"] >> 8)  & 0xff,
             source["_token"],
            +source["apng"],
            (source["width"]  >> 8) & 0xff,
             source["width"]        & 0xff,
...
function BinaryPacker0x0003_unpack(source, // @arg Uint8Array
                                   head) { // @arg Object = null
                                           // @ret Object - { _token, apng, width, height, loopCount, usePosterFrame, backgroundColor, frameData }
//{@dev
